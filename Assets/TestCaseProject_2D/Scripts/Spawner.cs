...
public class Spawner : MonoBehaviour
{
    ...
    void SpawnBlock()
    {
        ...
        // Eðer Boss savaþýndaysak bloklar sadece sað veya sol kenarlardan düþsün
        if (GameplayManager.Instance != null && GameplayManager.Instance.isBossPhase)
        {
            // %50 ihtimalle sol kenar, %50 ihtimalle sað kenar
            if (Random.value > 0.5f)
            {
                randomX = Random.Range(-2.5f, -1.5f); // Sol kenar
            }
            else
            {
                randomX = Random.Range(1.5f, 2.5f); // Sað kenar
            }
        }
        else
        {
            // Normal oyunda tüm ekrandan düþebilir
