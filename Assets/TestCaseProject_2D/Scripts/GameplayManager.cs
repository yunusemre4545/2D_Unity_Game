...
public class GameplayManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI bossFightText; // Boss Fight yazýsý için yeni deðiþken

    // Hit-Stop Mekanizmasý
    public System.Collections.IEnumerator DoHitStop(float duration)
    {
        Time.timeScale = 0f; // Oyunu tamamen dondur
        yield return new WaitForSecondsRealtime(duration); // Gerçek zamanda (game time dursa bile) kýsa bir süre bekle
        Time.timeScale = 1f; // Oyunu tekrar normal akýþýna döndür
    }
    void Awake()
    {
    ...
    void Start()
    {
        ...
        if (bossFightText != null) bossFightText.gameObject.SetActive(false); // Baþlangýçta yazýyý kapalý tut
    ...
    private IEnumerator StartBossPhase()
    {
        ...
        // 3 saniyelik sessizlik baþlarken BOSS FIGHT yazýsýný ekranda göster
        ...
        // 3 saniye bittiðinde Boss gelirken yazýyý tekrar kapat
    ...
    public void AddDestroyedBlock()
    {
        ...
        if (destroyedBlockCount % 4 == 0)
        {
            rocketCount++;
            UpdateRocketUI();
            Debug.Log("4 blok kýrýldý! +1 Roket kazanýldý.");
        }
    }
    ...
    public void TakeDamage()
    {
        ...
        // HÝT-STOP: Hasar aldýðýmýz an oyun 0.12 saniyeliðine ansýzýn dondurulsun!
